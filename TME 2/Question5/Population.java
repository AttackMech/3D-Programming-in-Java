...
*   Description: A collection of various objects for display in a 3D envirionment.  Two of the objects are textured.
...
*   adjusted them so the do not overlap.  One of the rectangular column objects is textured to look like bricks, one of the sphere
*   objects is textured to look like a tennis ball.
*
*/ 
...
public class Population
{
    ...
    Population (Canvas3D observer) {
        ...
        // floating diamond shape close to inital viewing position
        ...
        // create and set texture values for tennis ball sphere
        ImageComponent2D tennis = getTexImage("TennisBallz.gif", observer);
        Texture2D textures = new Texture2D(Texture2D.BASE_LEVEL, Texture2D.RGB, tennis.getWidth(), tennis.getHeight());
        textures.setImage(0, tennis);
        textures.setEnable(true);
        
        // a red sphere close to front wall near the top, textured to appear as a tennis ball
        Appearance appsph2 = new Appearance();
        appsph2.setColoringAttributes(new ColoringAttributes(new Color3f(1f, 1f, 0f), 1));
        appsph2.setTexture(textures); // applies texture values
        Sphere sph2 = new Sphere(1.5f, Sphere.GENERATE_TEXTURE_COORDS, appsph2);  // sphere radius 1.5, colour red
       ...
       // a black cube in the bottom right corner, close to the front wall
        ...
        // create and set texture values for brick column
        ImageComponent2D bricks = getTexImage("brick.gif", observer);
        Texture2D texture = new Texture2D(Texture2D.BASE_LEVEL, Texture2D.RGB, bricks.getWidth(), bricks.getHeight());
        texture.setImage(0, bricks);
        texture.setEnable(true);

        // a cyan rectangular column in the lower right, near the front wall, textured to appear as a brick structure
        Appearance appcol = new Appearance();
        appcol.setColoringAttributes(new ColoringAttributes(new Color3f(0f, 1f, 1f), 1));
        appcol.setTexture(texture); // applies texture settings
        Box column = new Box(1f, 4f, 1f, Box.GENERATE_TEXTURE_COORDS, appcol);  // column with specified dimensions, color cyan       
        ...
        bg.addChild(tgSphere2);  // tennis ball object
        bg.addChild(tgc1);
        bg.addChild(tgc2);
        bg.addChild(tgcol);  // brick column object
        bg.addChild(tgcyl1);
        bg.addChild(tgcyl2);
    ...
    private ImageComponent2D getTexImage(String filename, Canvas3D observer) {
        ...
        return image;
    } // end of method getTexImage(String, Canvas3D)
