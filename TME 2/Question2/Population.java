...
*   each object.  Each object is placed randomly throughout the room.  The second (red) sphere is able to be manipulated by returning
*   its associated TransformGroup.
*
*/ 
...
{
    private static final float radius = 1.5f;  // radius for moveable sphere
    
    private BranchGroup bg;  // basis of the scene graph for all geometry to be added
    private TransformGroup tgSphere2;  // TransformGroup to manipulate sphere position
    ...
    Population () {
        ...
        // this will be the sphere object that can be manipulated by the user
        Appearance appsph2 = new Appearance();
        appsph2.setColoringAttributes(new ColoringAttributes(new Color3f(1f, 0f, 0f), 1));
        Sphere sph2 = new Sphere(radius, appsph2);  // sphere radius 1.5, colour red
        
        tgSphere2 = new TransformGroup();
        Transform3D t3dSphere2 = new Transform3D();

        t3dSphere2.setTranslation(new Vector3f(5f, 5f, 10f));
        tgSphere2.setCapability(tgSphere2.ALLOW_TRANSFORM_WRITE);  // allows object to be changed in live scene
       ...
       // a black cube in the bottom right corner, close to the front wall
        ...
        bg.addChild(tgSphere2);  // moveable sphere object
        bg.addChild(tgc1);
        bg.addChild(tgc2);
        bg.addChild(tgcol);
        bg.addChild(tgcyl1);
    ...
    // returns the TransformGroup to manipulate the moveable Sphere object
    public TransformGroup getSphere() {
        return (tgSphere2);
    } // end of method getSphere()
    
    // returns the radius of the moveable sphere for calculation in movement of the object
